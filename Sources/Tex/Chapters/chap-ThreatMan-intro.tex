...
Je propose de continuer d'utiliser le modèle organisé autour de quatre volets (Surveiller, Détecter, Alerter, s'entrainer) \uref{Défendre les actifs}{lbl:varmenaces}  pour structurer la présentation.
...
On y trouvera l'application concrète des arbres d'attaques popularisés par un des plus célèbre cyber expert \wikipedia{Bruce Schneier} \ucite{schneier1999attack} qui est présentée de manière succincte  dans le  chapitre \nameref{sec:arbreattaques}
...
\subsubsection{Arbre d'attaques} \label{sec:arbreattaques}
