...
Dans le domaine informatique et télécom, le terme log est généralement un fichier, une base de données ou tout autre moyen de stocker des information, ici le stockage d'un historique d'événements qu'un logiciel ou un système souhaite \g{tracer}. Ce mot qui est le diminutif  de logging, est traduit en français par "journal". Le log est donc un journal horodaté, qui stocke temporellement les différents événements qui se sont produits sur un un logiciel, un ordinateur, un serveur, etc. Il permet ainsi d'analyser avec une fréquence programmer (heure par heure,  minute par minute, etc) l'activité  d'un processus technique.  

\upicture{../Tex/Pictures/img-log}{Sources de log}{0.7}{lbl_log}
