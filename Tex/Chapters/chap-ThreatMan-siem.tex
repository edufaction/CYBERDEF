...
Séquiper d’une solution de type SIEM nécessite un investissement conséquent en raison de la complexité de sa mise en œuvre. Toutefois, bien qu’initialement destiné aux grandes entreprises, le SIEM peut être déployer dans tous les types d’organisations, même les plus petits. Toutefois il ne faudra pas oublier que la configration d'un SIEM (création de scénario de détection (USE CASE)) demande de la ressource et des compétences spécifique.
...
\subsubsection{Construire les règles}
...
Le Gartner positionne régulièrement des produits et services dans son magic Quadran. en 2019, Splunk, IBM QRadar et LogRythm NextGen SIEM sont tourjours bien positionnés.
